...
export default function Hero3DViewer({
  ...
  const [panoramaUrl, setPanoramaUrl] = useState<string | undefined>(undefined)
  ...
  useEffect(() => {
    ...
    async function loadModel() {
      ...
      try {
        ...
        console.log('[Hero3DViewer] Model data received, model fields:', {
          glbUrl: data.model?.glbUrl,
          glbModelUrl: data.model?.glbModelUrl,
          model: data.model?.model,
          keys: data.model ? Object.keys(data.model) : null,
        })
        
        const url = data.model?.glbModelUrl || data.model?.model || data.model?.glbUrl || ''
        if (url) {
          ...
          setModelUrl(url)
          const panorama = data.model?.panorama || data.designs?.[0]?.bg_webp || data.designs?.[0]?.bgWebp
          setPanoramaUrl(panorama || undefined)
  ...
  if (!mounted || loading || !modelUrl || typeof window === 'undefined') {
    return (
  ...
  return (
    <div className="w-full h-full" style={{ position: 'relative' }}>
      ...
      {!modelUrl && !loading && (
        <div className="w-full h-full flex items-center justify-center bg-black">
          <div className="text-white text-sm opacity-50">No model URL available</div>
        </div>
      )}
    </div>
  )
