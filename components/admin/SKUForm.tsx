...
export default function SKUForm({ sku, onSuccess }: SKUFormProps) {
  ...
  const handleFileUpload = async (file: File, type: 'images' | 'videos' | 'description_file' | 'thumbnail') => {
    try {
      const formData = new FormData()
      formData.append('file', file)
      formData.append('type', type === 'images' ? 'images' : type === 'videos' ? 'videos' : 'documents')
    ...
    <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6 space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          {media.thumbnail && (
            <div className="mt-2">
              <img src={media.thumbnail.url} alt="Thumbnail" className="h-20 w-20 object-cover rounded" />
            </div>
          )}
        </div>
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {media.images.map((img, idx) => (
              <div key={idx} className="relative">
                <img src={img.url} alt={`Image ${idx + 1}`} className="h-20 w-20 object-cover rounded" />
                <button
                  type="button"
